...
The goal of `logician` is to do logic programming inspired by datalog/prolog in R.
It is written in R without any external dependencies. It targets interactive use
...
The general idea is query a database of facts and rules and ask questions.
...
This is all work in progress, still a bit hacky at some points but usable.
...
2. a rule that determines if there exists a path between two nodes.
    ...
    * if `A` is connected to an intermediate node `X` and there exists a path from
      `X` to `B`.
